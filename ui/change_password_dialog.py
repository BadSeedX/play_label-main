...
class changePasswordDialog(QDialog):
    ...
    def setUpUI(self):
        ...
        self.layout.addRow(self.workerIdLabel, self.workerIdEdit)
        ...
        self.workerIdLabel.setFont(font)
        ...
        font.setPixelSize(16)
        self.workerIdEdit.setFont(font)
        ...
        # 设置长度
        self.workerIdEdit.setMaxLength(10)
        ...
        self.workerIdEdit.setValidator(pValidator)
    ...
    def changePasswordButtonClicked(self):
        workerId = self.workerIdEdit.text()
        oldPassword = self.oldPasswordEdit.text()
        password = self.passwordEdit.text()
        confirmPassword = self.confirmPasswordEdit.text()
        if (workerId == "" or oldPassword == "" or password == "" or confirmPassword == ""):
        ...
        db.open()
        query = QSqlQuery()
        sql = "SELECT * FROM Worker WHERE WorkerId='%s'" % workerId
        query.exec_(sql)
        # 如果用户不存在
        if (not query.next()):
            print(QMessageBox.warning(self, "警告", "该用户不存在，请重新输入", QMessageBox.Yes, QMessageBox.Yes))
            self.WorkerIDEdit.clear()
            return
            # 如果密码错误
        hl = hashlib.md5()
        hl.update(oldPassword.encode(encoding='utf-8'))
        md5password = hl.hexdigest()
        sql = "SELECT * FROM Worker WHERE Password='%s' AND workerId='%s'" %(md5password,workerId)
        query.exec_(sql)
        if (not query.next()):
        ...
        md5password = hl.hexdigest()
        sql="UPDATE Worker SET Password='%s' WHERE WorkerID='%s'"%(md5password,workerId)
        query.exec_(sql)
        db.commit()
...
if __name__ == "__main__":
    ...
    app.setWindowIcon(QIcon("../res/images/MainWindow_1.png"))
